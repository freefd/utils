    ...
    if ($sshProcesses -is [system.array]) {
        foreach ($process in $sshProcesses) {
            if ($null -ne $process.CommandLine -And $process.CommandLine.Contains($sshPortForwardString)) {
                WriteLog $mainConfig['logFile'] '[!] Another SSH process',
                                    $process.ProcessId, 'with same port forwarding option is running'
                Exit 67
            }
        }
    } else {
        if ($null -ne $sshProcesses.CommandLine -And $sshProcesses.CommandLine.Contains($sshPortForwardString)) {
            WriteLog $mainConfig['logFile'] '[!] Another SSH process',
                                $sshProcesses.ProcessId, 'with same port forwarding option is running'
            Exit 67
        }
    }
}

$interfaceIndex = (Get-NetConnectionProfile | Where-Object {$_.NetworkCategory -ne 'DomainAuthenticated'} | Select-Object -ExpandProperty InterfaceIndex)
if (!$interfaceIndex) {
    WriteLog $mainConfig['logFile'] '[!] OS is offline, please connect to a network'
    Exit 67
} elseif ($interfaceIndex -is [system.array]) {
    WriteLog $mainConfig['logFile'] '[!] OS has multiple network connections'
    Exit 67
}

$currentIPv4 = (Get-NetIPAddress -AddressFamily IPv4 -InterfaceIndex $interfaceIndex | Select-Object -ExpandProperty IPAddress)

if (!$currentIPv4) {
    WriteLog $mainConfig['logFile'] '[!] Cannot find active IPv4 address for expected', [string]$mainConfig['expectedInterfaceAliases'], 'network interfaces' 
    Exit 71
}
...
foreach ($range in $mainConfig['allowedIPv4Ranges']) {
    if (([version]$range[0]) -lt ([version]$currentIPv4) -and ([version]$currentIPv4) -lt ([version]$range[1])) {
        ...
        WriteLog $mainConfig['logFile'] "[+] Starting SSH tunnel with port forwarding: ", (-join([string]$mainConfig['sshClient']['tunnelRemoteIP'],
                                                ':', [string]$mainConfig['sshClient']['tunnelRemotePort'],
